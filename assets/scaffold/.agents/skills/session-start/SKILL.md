...
This skill replaces the legacy startup command concept. That reference is migration context only; do not invoke slash commands or read any live legacy runtime paths.
...
Do not use legacy runtime state files, Claude session IDs, Claude transcript paths, or Claude-only command syntax.
