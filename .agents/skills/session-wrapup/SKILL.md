...
This skill replaces the legacy `/wrapup` closeout concept. That reference is migration context only; do not invoke slash commands or read any live Claude runtime paths.
...
Do not use `.claude/handoff.yaml`, `.claude/changelog.yaml`, Claude session IDs, Claude transcript paths, TaskCreate/TaskUpdate, or Claude-only command syntax.
