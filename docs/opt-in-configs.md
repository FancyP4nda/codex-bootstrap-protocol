...
1. Review hook examples under the installed `.codex/hooks/` directory when T004 has added the scaffold assets.
...
1. Review examples under `.codex/rules/` when installed by the scaffold.
...
1. Confirm `.agents/skills/minion/SKILL.md` exists after the skill bead lands.
2. Confirm the report-only worker custom agent exists after the custom-agent bead lands.
